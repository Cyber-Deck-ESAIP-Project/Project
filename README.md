...
**5. Active Deception (FTP Honeypot)**
...
**6. Password Auditor (Brute-Force)**
...
**18. Airspace Topology Router**
*   **Where:** Any recon scan.
*   **How to Test:** Run a LAN or WiFi sweep.
*   **Real-time Check:** Look at the main Python background terminal (where you ran `launcher.py`). The controller logs will prove that the thread gracefully bundled the raw JSON, hit the `EventBus`, updated the `AppState`, and saved to `logs/history.json` safely.

---

## 🛠️ VI. Deployment (Kali Linux / Raspberry Pi)
...
ExecStart=/home/kali/Cyber_Deck-ESAIP/.venv/bin/python3 /home/kali/Cyber_Deck-ESAIP/launcher.py
Restart=on-failure
RestartSec=5
...
Your Pi will now automatically boot directly into the CyberDeck OS dashboard on every startup!
